...
bm_full_rand <- brm(mean_click ~ ageinyears + touchscreen + household + younger_children + children + (touchscreen + household + younger_children + children + ageinyears |community), 
                              data   = scdata, 
                              family = lognormal(),
                              warmup = 2000, 
                              iter   = 4000, 
...
model_weights(bm_base, 
            bm_octs_fix, criterion = "loo")%>%as_tibble(rownames = "model")
...
cpost <- fitted(bm_cts_rand_int, newdata = ncdata, re_formula =~(children|community))%>%
  as_tibble() %>%
  bind_cols(ncdata)
