...
raw_demographics_namibia <- bind_rows(
  read_xlsx("../../raw_data/gafo-cc/namibia/Data FarmSix 2022.xlsx", sheet = 1)%>%mutate(ID = paste0("H",str_pad(ID, 7, pad = "0"))),
  read_xlsx("../../raw_data/gafo-cc/namibia/Data Khwe 2022.xlsx", sheet = 1)%>%mutate(ID = paste0("K",str_pad(ID, 7, pad = "0")))%>%rename(sex = `...7`)
  )
...
data_zambia <- tidy_demographics_zambia%>%
  ...
  mutate(community = "chimfunshi_kumwenda")
```
