...
```{r demographics}
raw_demographics_uganda <- read_xlsx("../../raw_data/gafo-cc/uganda/gafo_uganda_participants.xlsx", sheet = 1)[-1, ]
...
exclude_uganda <- tidy_demographics_uganda%>%filter(younger_children >= children) %>% pull(subjid)
```

```{r}
data_uganda <- tidy_demographics_uganda%>%
  left_join(tidy_data_uganda%>%rename_all(tolower),by = c("subjid"))%>%
  filter(!subjid %in% exclude_uganda)%>%
  distinct(subjid, trialnr, .keep_all = TRUE)
```
...
methods_data <- merge_raw_data%>%
  select(subjid, community, age_group, ageinyears, trialtype, voiceover, trialnr, targetposition,targetcentralityx, clickdistfromtargetcenterx)
