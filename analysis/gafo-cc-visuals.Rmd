...
```{r}
im <- readPNG("../visuals/gazemodel-cc-center.png")
im2 <- matrix(rgb(im[,,1],im[,,2],im[,,3], im[,,4] * 0.5), nrow=dim(im)[1])

sim_dat <- tibble(
  targetposition = factor(c(1:10)), 
  mean = c(550,350,225, 150 , 100, 100, 150,225,350,550),
  sd = c(seq(100,50, length.out = 5), seq(50, 100, length.out = 5)),
  col = c("no","no","no","no","yes","yes","no","no","no","no")
)%>%
  expand_grid(id = as.character(c(1:50)))%>%
  rowwise()%>%
  mutate(impr = rnorm(1, mean, sd))

ggplot(sim_dat, aes(x = targetposition, y= mean, col = col))+
  background_image(im2) + 
  geom_point(size = 3)+
  geom_jitter(aes(y = impr), alpha = 0.1, width = 0.1)+
  scale_color_manual(values = c("black", "firebrick"), guide = "none")+
  ylim(0, 1000)+
  labs(x = "Target position (binned)", y = "Imprecision (simulated)")+
  theme_few()


ggsave("../visuals/target_position_model_center.png", width = 6.8, height = 4)


im3 <- readPNG("../visuals/gazemodel-cc-side.png")
im4 <- matrix(rgb(im3[,,1],im3[,,2],im3[,,3], im3[,,4] * 0.5), nrow=dim(im3)[1])

sim_dat2 <- tibble(
  targetposition = factor(c(1:10)), 
  mean = c(550,350,225, 150 , 100, 100, 150,225,350,550),
  sd = c(seq(100,50, length.out = 5), seq(50, 100, length.out = 5)),
  col = c("yes","no","no","no","no","no","no","no","no","yes")
)%>%
  expand_grid(id = as.character(c(1:50)))%>%
  rowwise()%>%
  mutate(impr = rnorm(1, mean, sd))
  
ggplot(sim_dat2, aes(x = targetposition, y= mean, col = col))+
  background_image(im4) + 
  geom_point(size = 3)+
  geom_jitter(aes(y = impr), alpha = 0.1, width = 0.1)+
  scale_color_manual(values = c("black", "firebrick"), guide = "none")+
  ylim(0, 1000)+
...
ggsave("../visuals/target_position_model_edge.png", width = 6.8, height = 4)
```
...
post_all <- fitted(bm_base, newdata = ndata_all, re_formula = NA)%>%
  as_tibble() %>%
  bind_cols(ndata_all)%>%
  mutate(ageinyears = ageinyears + min(data$ageinyears))
...
pagg_dev <- ggplot()+
  ...
  #geom_smooth(data = post, aes(x = ageinyears, y = Estimate, ymin = Q2.5, ymax = Q97.5, fill =community, col = community), stat = "identity", alpha = .1, size = 0)+
  #geom_line(data = post, aes(x = ageinyears, y = Estimate, fill =community, col = community), stat = "identity", alpha = .4)+
...
pind <- ggplot()+ 
 geom_point(data = ddata, aes(x = ageinyears+ min(data$ageinyears), y = mean_click, col = community), alpha = 0.25)+
  ...
  facet_wrap(~community, nrow = 3)#+guides(col = "none", fill = "none")
```

```{r}
plot_grid(
  pagg_dev, pind, rel_widths = c(1,1.5), align = "h"
)
```


```{r}
ggsave("../visuals/dev2.png", width = 12, height = 4, bg = "white", scale = 1.25)
```
...
ptagg <- ggplot()+
  ...
  #geom_pointrange(data =tc_com, aes(x = targetposition, y = mean, ymin = ci_lower, ymax = ci_upper, col = community),pch = 1, alpha = 0.2, position = position_dodge(width = 0.25))+
...
```{r}
plot_grid(
  ptagg, ptind, rel_widths = c(1,1.5), align = "h"
)
```

```{r}
ggsave("../visuals/tc2.png", width = 12, height = 4, bg = "white", scale = 1.25)
```
...
```
```{r}
ggsave("../visuals/fig2_alt2.png", width = 12, height = 8, bg = "white", scale = 1.25)
```
...
iclick = data%>%
  group_by(community, targetposition)%>%
  summarise(click_dist_mean = mean(clickdistfromtargetcenterx))
  mutate(click_dist_mean = ifelse(targetposition< 6, -click_dist_mean, click_dist_mean))

piclick <- data%>%
  mutate(fill1 = ifelse(targetcenterx < 960, -clickdistfromtargetcenterx, clickdistfromtargetcenterx), 
         fill = ifelse(fill1 < 0, "yes", "no"))%>%
  ggplot(aes(x = clickdistfromtargetcenterx, y = community))+
  #geom_density_ridges(data = .%>%filter(fill == "yes"), alpha = 0.5, col = NA, fill = "#85AD00",bandwidth = 200)+
  #geom_density_ridges(data = .%>%filter(fill == "no"), alpha = 0.5, col = NA, fill = "grey",bandwidth = 200)+
  geom_density_ridges(alpha = .75,  bandwidth = 200, fill = "grey", col = "white")+
  geom_vline(xintercept = 0, lty = 1, size = 1)+
  facet_grid(~factor(targetposition))+
  labs(x = "Click distance from target", y = "")+
  scale_x_continuous(breaks = c(0), limits = c(-1500, 1500))+
  scale_y_discrete(limits=rev)+
  #scale_fill_manual(values = c("grey","#85AD00"))+
  scale_color_manual(name = "", guide = "none", values = colors)+
  geom_point(data = iclick, aes(x = click_dist_mean, y = community), col = "firebrick", size = 1, stroke = 1)+
  theme_few()
```
...
```{r}
ggsave("../visuals/fig2_alt3.png", width = 12, height = 10, bg = "white", scale = 1.4)
```
...
ggplot(data = selWorld, mapping = aes(x = long, y = lat, group = group)) + 
  coord_map("moll")+
  geom_polygon(aes(fill = fill), col = "white", linewidth = 0.05) +
  scale_fill_manual(breaks = c("out", "in"), values = c("grey", "#DBD56E"), guide = "none")+
  new_scale("fill") +
  geom_point(pch = 21, data = data_map, aes(x = loc_long, y = loc_lat, fill = Participants), col = "black", inherit.aes = F, size = 4, stroke = 0.5)+
  ...
  plain+
  theme(legend.position = c(0.2, 0.25), legend.background = element_blank())
```
```{r}
ggsave("../visuals/map.png", width = 8, height = 4, bg = "white", scale = 1.3)
```
...
gWorld <- world%>%
  filter(region != "Antarctica")%>%
  mutate(fill = ifelse(region %in% c("Turkey", "Germany", "Kenya"), "out", "in"))%>% 
  filter(! long > 180)
```

```{r}
ggplot(data = gWorld%>%filter(! long  < -30, long < 60,  lat > -40, lat < 70 ), mapping = aes(x = long, y = lat, group = group)) + 
  coord_map("moll")+
  geom_polygon(aes(fill = fill), col = NA, linewidth = 0) +
  scale_fill_manual(breaks = c("out", "in"), values = c( "black", "#f2f2f2"), guide = "none")+
  plain

library(rworldmap)
cont <- map_data(coastsCoarse)
plot(coastsCoarse)
```

```{r}
ggsave("../../../../../../Desktop/map.png", width = 4, height = 4, scale = 1.3)
```

```{r}
gWorld2 <- world%>%
  filter(region != "Antarctica")%>%
  mutate(fill = ifelse(region %in% c("Turkey", "Germany", "Kenya", "USA", "Mexico", "Argentina", "Colombia", "Morocco", "Nigeria", "Namibia", "India", "China", "Malaysia", "Australia", "Zambia", "Japan", "Spain", "UK", "Republic of Congo", "Sweden", "Turkmenistan"), "out", "in"))%>% 
  filter(! long > 180)
```

```{r}
ggplot(data = gWorld2, mapping = aes(x = long, y = lat, group = group)) + 
  coord_map("moll")+
  geom_polygon(aes(fill = fill), col = NA, linewidth = 0) +
  scale_fill_manual(breaks = c("out", "in"), values = c( "#DBD56E", "grey"), guide = "none")+
  plain
```

```{r}
ggsave("../../../../../../Desktop/map2.png", width = 12, height = 8, scale = 1)
```
...
```{r}
meand <- ddata%>%
  filter(community %in% c("Leipzig (Germany)", "Akure (Nigeria)", "Malatya (Turkey)"))%>%
  group_by(community)%>%
  tidyboot_mean(col = mean_click)

ggplot()+
  #geom_point(data = ddata%>%filter(community %in% c("Akure (Nigeria)")), aes(x = ageinyears+ min(data$ageinyears), y = mean_click, col = community), alpha = 0.25)+
    geom_point(data = ddata%>%filter(community %in% c("Leipzig (Germany)", "Akure (Nigeria)", "Malatya (Turkey)")), aes(x = ageinyears+ min(data$ageinyears), y = mean_click, col = community), alpha = 0.25)+
  #geom_smooth(data = post%>%filter(community %in% c("Akure (Nigeria)")), aes(x = ageinyears, y = Estimate, ymin = Q2.5, ymax = Q97.5, fill =community, col = community), stat = "identity", alpha = .25)+
    geom_smooth(data = post%>%filter(community %in% c("Leipzig (Germany)",  "Akure (Nigeria)", "Malatya (Turkey)")), aes(x = ageinyears, y = Estimate, ymin = Q2.5, ymax = Q97.5, fill =community, col = community), stat = "identity", alpha = .25)+
  #geom_ysidedensity(data = ddata%>%filter(community %in% c("Akure (Nigeria)")), aes(y = mean_click, fill = community),col = "white",alpha = 0.5)+
    #geom_ysidedensity(data = ddata%>%filter(community %in% c("Leipzig (Germany)",  "Akure (Nigeria)", "Malatya (Turkey)")), aes(y = mean_click, fill = community),col = "white",alpha = 0.5)+
  geom_ysidepoint(data = meand, aes(y =mean, col = community, x = community), pch = 4, orientation = "x", size = 2, stroke = 2)+
  geom_ysidesegment(data = meand, aes(y =ci_lower, yend = ci_upper , col = community, x = community, xend = community), size = 1)+
  theme_classic()+
    scale_x_continuous(limits = c(2,9))+
  scale_y_continuous(limits = c(0,1000))+
  scale_fill_colorblind(name = "Community", guide = "none")+
  scale_color_colorblind(name = "Community", guide = "none")+
  labs(x = "Age", y="Cognitive measure (Imprecision)")+
  theme(legend.position = "right")+
...
```{r}
ggsave("../../../../../../Desktop/dev4.png", width = 10, height = 6, scale = 1)
```