...
ddata <- data%>%
  ...
  group_by(subjid, community, ageinyears)%>%
...
ggplot()+
  ...
  theme_bw()+
  geom_point(data = ddata, aes(x = ageinyears+ min(data$ageinyears), y = mean_click, col = community), alpha = 0.5)+
  scale_color_ptol(name = "Community")+
...
```{r}
ggsave("../visuals/dev.png", width = 10, height = 6, bg = "white")
```
...
ggplot(data = selWorld, mapping = aes(x = long, y = lat, group = group)) + 
  ...
  scale_fill_manual(breaks = c("out", "in"), values = c("grey", "#12664F"), guide = "none")+
  new_scale("fill") +
...
```
```{r}
ggsave("../visuals/map.pdf", width = 10, height = 6, bg = "white")
```
