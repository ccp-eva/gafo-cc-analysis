...
```{r}
bm_inward_click <- brm(inwardclick ~ ageinyears + targetcentralityx + (targetcentralityx|subjid),
                              data   = mdata,
                              family = bernoulli(),
                              warmup = 2000,
                              iter   = 6000,
...
saveRDS(bm_inward_click, "../saves/bm_inward_click.rds")
```
...
In the second analysis focused on signatures in the data that suggest that children in all cultural settings use similar cognitive processes to infer the location of the balloon. The first -- somewhat weaker -- signature was simply an increase in performance with age. The second signature was a strong effect of target centrality. That is, the further away from the center the balloon lands, the more imprecise should participants be. This second signature is derived from the model by the cognitive model introduced in @prein2023variation. It predicts that trials in which the agent looks further away from the center (i.e. to the left or right side of the screen) result in higher levels of imprecision compared to trials in which the agent looks closer to the middle. The third prediction is derived from the same model and predicts that for any given location where the target lands, clicks are not normally distributed around the true location but the distribution is skewed "inwards", that is, towards the center. This effect should be stronger the further away from the center the target lands.
